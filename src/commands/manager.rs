...
impl ManagerCommands {
    ...
    ) -> Result<Vec<models::collection::Collection>, Box<dyn std::error::Error>> {
        ...
        match self {
            ...
            Self::Delete {
                ...
                if let Some(col) = collections.iter_mut().find(|c| c.name == *collection) {
                    ...
                    } else if let Some(requests) = col.requests.as_mut() {
            ...
            Self::Copy {
                ...
                if let Some(col) = collections.iter().find(|c| c.name == *collection) {
                        ...
                        .and_then(|r| r.iter().find(|r| r.name == *endpoint))
                    {
                        ...
                        if *to_col {
                            if let Some(to_collection) =
                                collections.iter_mut().find(|c| c.name == *new_name)
                            {
                                let mut new_requests =
                                    to_collection.requests.clone().unwrap_or_default();
                            ...
                            if let Some(col_mut) =
                                collections.iter_mut().find(|c| c.name == *collection)
                            {
            ...
            Self::Update {
                ...
                let collections = Self::load_collections()?;
                let mut found = false;
                let collections: Vec<models::collection::Collection> = collections
                    .into_iter()
                    .map(|mut c| {
