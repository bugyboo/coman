...
pub enum ManagerCommands {
    ...
    Col { 
        ...
        #[clap(
            short = 'H', 
            long = "header",
            value_parser = helper::parse_header,
...
pub fn get_endpoint_command(collection: &str, endpoint: &str) -> Option<RequestCommands> {
    let collections = load_collections().unwrap_or_default();
...
pub fn run(command: ManagerCommands, verbose: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match command {
        ManagerCommands::List { col, verbose } => {
        ...
        ManagerCommands::Delete { collection, endpoint } => {
            let collections = load_collections()?;
            let mut found = false;
            let collections: Vec<models::collection::Collection> = collections.into_iter().map(|c| {
                if c.name == collection {
                    ...
                    if endpoint.is_empty() {
                        ...
                        let confirm = helper::confirm("Are you sure you want to delete this collection?");
                        if confirm {                            
                            return None;
                        }
